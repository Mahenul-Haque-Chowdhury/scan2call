...
import { TAG_TYPE_LABELS, type TagType } from '@scan2call/shared';
...
const TAG_STATUS_BADGE_VARIANTS: Record<string, 'success' | 'warning' | 'error' | 'neutral'> = {
  ACTIVE: 'success',
  INACTIVE: 'warning',
  LOST: 'error',
  FOUND: 'success',
...
function getTagTypeLabel(type: OwnedTag['type']) {
  return TAG_TYPE_LABELS[type as TagType] ?? String(type).replace(/_/g, ' ').toLowerCase();
}
...
export default function AdminUserDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setTimeout(() => router.push('/office/users'), 1500);
    } catch (err) {
  ...
  if (error || !user) {
    return (
      <div>
        <Link href="/office/users" className="inline-flex items-center gap-1.5 text-sm text-text-dim transition-colors hover:text-text">
        ...
        <div className="rounded-lg border border-border bg-surface p-6">
          ...
          <div className="mt-6 border-t border-border pt-5">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-sm font-medium text-text">Owned Tags</h3>
              <span className="text-xs text-text-dim">{user.tags?.length ?? 0} total</span>
            </div>

            {user.tags?.length ? (
              <div className="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">
                {user.tags.map((tag) => (
                  <div key={tag.id} className="rounded-md border border-border bg-surface-raised p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs text-text-dim">
                          {getTagTypeLabel(tag.type)} - {tag.token}
                        </p>
                      </div>
                      <Badge variant={TAG_STATUS_BADGE_VARIANTS[tag.status] ?? 'neutral'}>
                        {tag.status}
                      </Badge>
                    </div>
                    <div className="mt-3 grid gap-2 text-xs text-text-dim sm:grid-cols-3">
                      <span>{tag._count?.scans ?? 0} scans</span>
                      <span>{tag.isLostMode ? 'Lost mode on' : 'Lost mode off'}</span>
                      <span>
                        {tag.activatedAt
              ...
              <div className="mt-3 rounded-md border border-dashed border-border bg-surface-raised p-4 text-sm text-text-dim">
                This customer does not own any tags yet.
              </div>
            )}
          </div>
        </div>
