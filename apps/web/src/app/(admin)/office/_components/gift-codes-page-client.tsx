...
import { Search, Copy, Tag, Camera, ScanLine, Trash2 } from 'lucide-react';
...
const STATUS_BADGE: Record<TagGiftCodeRow['status'], string> = {
...
interface GiftCodesPageClientProps {
  /** Only tag gifts remain; kept optional for call-site compatibility. */
  mode?: 'tag';
}

export function GiftCodesPageClient(_props: GiftCodesPageClientProps = {}) {
  void _props;
    ...
    <div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-border bg-surface p-6">
          ...
          <p className="mt-1 text-sm text-text-muted">Create a one-time code for gifting one physical tag.</p>
          ...
          <div className="mt-5 grid gap-4">
            ...
            <Alert variant="info">After creating the code, scan the physical tag QR code or paste its 12-character token to reserve that exact tag.</Alert>
        ...
        <div className="rounded-lg border border-border bg-surface p-6">
          ...
          <p className="mt-1 text-sm text-text-muted">Bind an inactive, unowned physical tag to an active gift code by scanning the tag QR code or entering its 12-character token.</p>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold uppercase tracking-widest text-text-dim">Physical tag token</label>
              <div className="mt-2 flex gap-2">
                ...
                <Button variant="secondary" onClick={handleStartTagScanner} loading={tagScannerOpening} icon={<Camera className="h-4 w-4" />}>
                  {tagScannerOpening ? 'Opening...' : tagScannerActive ? 'Scanning...' : 'Scan'}
                </Button>
              </div>
            </div>
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-black/95">
              <div className="relative aspect-4/3 w-full">
                <video ref={reserveVideoRef} muted playsInline autoPlay className="h-full w-full object-cover" />
                {!tagScannerActive && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/70 px-6 text-center text-white">
                    ...
                    <p className="text-sm text-white/80">Open the camera and point it at the physical tag QR code.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {tagScannerActive && (
              <Button variant="ghost" onClick={stopTagScanner}>
                Stop Camera
              </Button>
            )}
