    ...
    async function fetchProduct() {
      try {
        const res = await fetch(
          `${API_BASE}/api/v1/products/${productSlug}`,
        );
        if (res.status === 404) {
            ...
            {sortedImages.length > 0 ? (
              <AnimatePresence mode="wait">
                <motion.img
                  ...
                  alt={
                    sortedImages[selectedImage]?.altText || product.name
                  }
        ...
        >
          ...
          <div className="mt-6 space-y-0 text-sm">
            {[
              ...
              ...(product.includesTagCount > 0 ? [{ label: 'Tags Included', value: String(product.includesTagCount) }] : []),
