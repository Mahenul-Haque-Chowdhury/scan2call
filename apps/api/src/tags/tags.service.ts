...
export class TagsService {
   ...
   * Tags are pre-generated by admin and claimed by scanning or entering the token.
   */
  async activate(userId: string, dto: ActivateTagDto): Promise<TagDetail> {
    ...
    if (!tag) {
      throw new NotFoundException('Tag not found. Please check the token and try again.');
    }
  ...
  async getPublicTagInfo(token: string): Promise<PublicTagInfo> {
    ...
    if (tag.status === 'INACTIVE') {
      throw new BadRequestException(
        'This tag has not been activated yet. Please visit scan2call.com to activate.',
      );
    }
